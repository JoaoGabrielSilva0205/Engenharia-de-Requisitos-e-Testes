# Use Case Diagram — Lab 5

## System boundary
- System name: EcoDoar Platform
- Slice covered: Gestão de Doações e Correspondência

## Actors (2–4)
- A1: Doador
- A2: Beneficiário
- A3: Sistema de Notificações

## Use cases (min. 6)
- UC-01: Criar anúncio de item
- UC-02: Editar anúncio de item
- UC-03: Alterar estado do item
- UC-04: Solicitar item disponível
- UC-05: Associar pedido a item disponível
- UC-06: Notificar utilizadores sobre correspondência

## Diagram file
- Path: `docs/diagrams/use_case_diagram.puml`
